...
Info card shows **Total Members** for vacation, emergency, and investment (ongoing, funded, and completed) from `project.totalJoinedMember` only. Missing or `0` hides the row (no pagination / `members[]` / creator floor). **View All Members (N)** and full-screen **Group Members (N)** (`/project/group-members`) use the same field; the `(N)` is omitted when the count is 0.
...
Group Leader **Continue contribution** (cancel open vote) is on the leader monitor only — not member/co-leader cast UI. Confirm → `POST …/closure-voting/cancel` (`{}` body) → reload `GET /projects/{id}` (source of truth) → pop to detail. Hidden at 50% of `totalJoinedMember` or when `voting.canContinueContributions` is false. 409 (`VoteParticipationThresholdReached` / `VoteWindowClosed` / `VoteAlreadyFinalized`) toasts mapped copy and reloads the monitor.
