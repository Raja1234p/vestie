...
| Dashboard Home | `GET /projects` (user list) — card amount: `potAmount` → `raisedAmount` → `totalContributed` → `viewerRefundAmount`. **Total Members** from list `totalJoinedMember` (missing/`0` → hidden). `memberCount` is eligible voters only. | Shimmer | Tab re-activate / reload flags |
| Discover | `GET /projects` (discover) — same **Total Members** (`totalJoinedMember`) as Home cards | Shimmer | Tab re-activate |
...
| Project detail | `GET /projects/{id}` — raised headline: pot/raised → `totalContributed` → `voting.viewerRefundAmount` / `project.viewerRefundAmount`. **Total Members** / **View All Members (N)** / **Group Members (N)** from `project.totalJoinedMember` only (missing/`0` → hidden, no pagination/`members[]` floor). | Shimmer | After contribute / announcement |
...
| Completed projects list | `GET /projects/completed?page=&pageSize=` — list key `completedProjects`; cancelled rows use `viewerRefundAmount` when pot/raised/contributions are zero. **Total Members** from `totalJoinedMember` (missing/`0` → hidden). | `ProjectCardShimmer` list | `PaginatedScrollListener` load-more |
...
| Project detail — leader View Success Votes | `GET /projects/{id}` → `voting` + `memberVotes[]` (Week 11) | Shimmer on monitor screen | Pull-to-refresh → same single detail call; legacy projects fall back to `GET …/closure-voting/active`. **Continue contribution** (GroupLeader only): confirm → `POST …/closure-voting/cancel` with `{}` → on **200** `{ cancelled, projectId, votingStatus: not_started }` → `reloadDetailAndWait` (`GET /projects/{id}` source of truth) → pop to detail. **409** (`VoteParticipationThresholdReached` / `VoteWindowClosed` / `VoteAlreadyFinalized`) or **404** `NoOpenVote`: toast mapped copy and reload monitor GET (hide button). Hidden at 50% of `totalJoinedMember` or when `canContinueContributions` is false. |
